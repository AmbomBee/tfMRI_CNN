...
class fMRIDataset(Dataset):
    ...
    def __get_label__(self, img_name):
        ...
        try:
            ...
            return (self.label_dict['label'][index], self.label_dict['label_ID'][index])
    ...
    def __getitem__(self, idx):
        ...
        label, label_ID = self.__get_label__(img_name)
        if label == None:
            print('label == None', flush=True)
        sample = {'fdata': fdata, 'label': label, 'label_ID': label_ID}
        if self.transform:
